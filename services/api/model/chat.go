...
func (c *Contract) GetChatList(db *pgxpool.Conn, ctx context.Context, param map[string]interface{}) ([]ChatGroupEnt, error) {
	...
	join members m on m.id = cg.created_by
...
func (c *Contract) GetChatHistoryByGroupCode(db *pgxpool.Conn, ctx context.Context, code string, param map[string]interface{}) (ChatGroupEnt, error) {
	...
	query := `
			select 
				...
				mi.title, mi.itin_code, o.order_type, o.order_code,
			...
			group by a.name, chat_group_code, a.cg_id, mi.id,cgr.total_member, chat_group_type, m.id , o.id`
	...
	err := db.QueryRow(ctx, query, paramQuery...).Scan(&gc.ID, &gc.Name, &gc.ChatGroupCode, &gc.ChatGroupType,
		...
		&gc.TotalMember, &itinTitle, &itinCode, &orderType, &orderCode,
...
func (c *Contract) GetStatusLeaveSeason(db *pgxpool.Conn, ctx context.Context, code string) (ChatGroupEnt, error) {
	var cg ChatGroupEnt

	err := pgxscan.Get(ctx, db, &cg, "select status,name from chat_groups where status = true and chat_group_code=$1 limit 1", code)
	return cg, err
}

func (c *Contract) UpdateStatus(tx pgx.Tx, ctx context.Context, code string, cg ChatGroupEnt) error {

	var ID int32

	sql := `UPDATE chat_groups SET status=$1 WHERE chat_group_code=$2  RETURNING id`

	err := tx.QueryRow(ctx, sql, cg.Status, code).Scan(&ID)
